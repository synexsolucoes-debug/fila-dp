...
import { ChevronRight, KeyRound, Lock, Pause, Play, RefreshCcw, RotateCw, ShieldX, SlidersHorizontal, Unlock } from "lucide-react";
...
export function IntegrationsFeature({ params, updateQuery }: FeatureProps) {
  ...
  function perform(row: Row, kind: string) {
    ...
    const labels: Row = { run: "Executar sincronização", retry: "Repetir execução", pause: "Pausar integração", resume: "Retomar integração", revoke_credential: "Revogar credencial", rotate_credential: "Rotacionar credencial" };
    const consequences: Row = { run: "Uma execução manual será enfileirada com chave de idempotência própria.", retry: "Uma nova execução será criada a partir da falha selecionada.", pause: "Novas sincronizações ficarão impedidas até retomada explícita.", resume: "A integração voltará a aceitar jobs se houver credencial e mapeamento ativos.", revoke_credential: "A credencial ativa será revogada imediatamente e a integração exigirá nova configuração.", rotate_credential: "A credencial anterior será revogada e o novo segredo será cifrado antes de ser persistido." };
