...
export async function POST(_request: Request, context: RouteContext) {
  ...
  try {
    ...
    const position = await d1.prepare("SELECT COALESCE(MAX(position), 0) AS max_position FROM cards WHERE list_id = ? AND archived = 0").bind(list.id).first<{ max_position: number }>();
