...
const requirementSelect = `SELECT r.*, p.name AS product_name, p.ca_number, p.ca_expires_on, p.product_expires_on,
  d.name AS department_name, po.name AS position_name, est.name AS establishment_name
  ...
  LEFT JOIN fdp_establishments est ON est.workspace_id = r.workspace_id AND est.company_id = r.company_id AND est.id = r.establishment_id`;
...
export async function GET(request: Request) {
  ...
  try {
    ...
    const [requirements, departments, positions, establishments] = await Promise.all([
      d1.prepare(`${requirementSelect} WHERE r.workspace_id = ? AND r.company_id = ?
        ORDER BY r.active DESC, COALESCE(est.name, ''), COALESCE(d.name, ''), COALESCE(po.name, ''), p.name`)
      ...
      d1.prepare(`SELECT id, name, code, status FROM fdp_positions
        WHERE workspace_id = ? AND company_id = ? ORDER BY status, name`)
        .bind(workspace.id, companyId).all<Record<string, unknown>>(),
      d1.prepare(`SELECT id, name, code, status FROM fdp_establishments
        WHERE workspace_id = ? AND company_id = ? ORDER BY status, name`)
        .bind(workspace.id, companyId).all<Record<string, unknown>>(),
    ]);
    return Response.json({
      requirements: requirements.results, departments: departments.results,
      positions: positions.results, establishments: establishments.results,
    });
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const row = {
      ...
      establishmentId: cleanText(body.establishmentId, 120) || null,
    ...
    const [product, department, position, establishment, duplicate] = await Promise.all([
      ...
      row.establishmentId ? d1.prepare("SELECT id FROM fdp_establishments WHERE workspace_id = ? AND company_id = ? AND id = ?")
        .bind(workspace.id, row.companyId, row.establishmentId).first() : Promise.resolve({ id: "all" }),
      d1.prepare(`SELECT id FROM fdp_epi_requirements WHERE workspace_id = ? AND company_id = ?
        AND department_id IS NOT DISTINCT FROM ? AND position_id IS NOT DISTINCT FROM ?
        AND establishment_id IS NOT DISTINCT FROM ? AND product_id = ?`)
        .bind(workspace.id, row.companyId, row.departmentId, row.positionId, row.establishmentId, row.productId).first(),
    ]);
    ...
    if (!establishment) throw ApiError.badRequest("A unidade não pertence à empresa selecionada.", "EPI_REQUIREMENT_ESTABLISHMENT_INVALID");
    if (duplicate) throw new ApiError(409, "EPI_REQUIREMENT_CONFLICT", "Já existe uma regra para este EPI, unidade, cargo e departamento.");

    await d1.batch([
      d1.prepare(`INSERT INTO fdp_epi_requirements
        (id, workspace_id, company_id, department_id, position_id, establishment_id, product_id, quantity,
         replacement_days, warning_days, active, notes, created_by, updated_by)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 1, ?, ?, ?)`)
        .bind(row.id, workspace.id, row.companyId, row.departmentId, row.positionId, row.establishmentId, row.productId,
