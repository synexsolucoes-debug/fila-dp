...
export async function PATCH(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const active = Object.hasOwn(body, "active") ? Boolean(body.active) : String(current.status) !== "inactive";
    ...
    if (requestedLifecycle === "archived") lifecycle = "archived"; else if (!active) lifecycle = "inactive";
    ...
    await d1.batch([
      ...
      prepareAuditEvent({ workspaceId: workspace.id, actorUserId: user.id, actorEmail: auth.user.email, action: lifecycle === "archived" ? "process.archived" : "process.updated", entityType: "process_definition", entityId: id, before: current, after: { name, description, objective, category, lifecycle, ownerDepartmentId, ownerUserId, isCorporate, companyIds }, requestId: request.headers.get("x-fila-dp-request-id") }),
    ]);
