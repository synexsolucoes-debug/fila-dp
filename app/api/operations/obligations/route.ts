...
import { enumOr, obligationTypes, validRequiredDate } from "@/lib/operations";
...
export async function POST(request: Request) {
  ...
  try {
    ...
    await d1.batch([
      d1.prepare(`INSERT INTO fdp_compliance_obligations (id, workspace_id, company_id, payroll_cycle_id, card_id, obligation_type, title, due_date, status, owner_user_id, recurrence_key, notes)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, 'open', ?, ?, ?)`).bind(id, workspace.id, companyId, cycleId, obligation.cardId, obligation.obligationType, title, dueDate, obligation.ownerUserId, recurrenceKey, obligation.notes),
