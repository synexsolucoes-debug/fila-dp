...
export async function POST(request: Request) {
  ...
  try {
    ...
    const id = crypto.randomUUID(); const recurrenceKey = cleanText(body.recurrenceKey, 180); const obligation = { id, companyId, payrollCycleId: cycleId, obligationType: enumOr(body.obligationType, obligationTypes, "other"), title, dueDate, status: "open", ownerUserId: cleanText(body.ownerUserId, 120) || null, cardId: cleanText(body.cardId, 120) || null, establishmentId: cleanText(body.establishmentId, 120) || null, notes: cleanText(body.notes, 1000), recurrenceKey,
    ...
    await d1.batch([
      d1.prepare(`INSERT INTO fdp_compliance_obligations (id, workspace_id, company_id, payroll_cycle_id, card_id, establishment_id, obligation_type, title, due_date, status, owner_user_id, recurrence_key, notes, protocol, evidence_url)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, 'open', ?, ?, ?, ?, ?)`).bind(id, workspace.id, companyId, cycleId, obligation.cardId, obligation.establishmentId, obligation.obligationType, title, dueDate, obligation.ownerUserId, recurrenceKey, obligation.notes, obligation.protocol, obligation.evidenceUrl),
