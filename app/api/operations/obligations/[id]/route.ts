...
export async function PATCH(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const next = { title: cleanText(body.title, 180) || String(current.title), dueDate: Object.hasOwn(body, "dueDate") ? validRequiredDate(body.dueDate) : String(current.due_date), status,
      ownerUserId: Object.hasOwn(body, "ownerUserId") ? cleanText(body.ownerUserId, 120) || null : current.owner_user_id,
      establishmentId: Object.hasOwn(body, "establishmentId") ? cleanText(body.establishmentId, 120) || null : current.establishment_id,
    ...
    await d1.batch([
      d1.prepare("UPDATE fdp_compliance_obligations SET title = ?, due_date = ?, status = ?, owner_user_id = ?, establishment_id = ?, notes = ?, protocol = ?, evidence_url = ?, completed_at = CASE WHEN ? = 'completed' THEN CURRENT_TIMESTAMP ELSE NULL END, updated_at = CURRENT_TIMESTAMP WHERE workspace_id = ? AND id = ?")
        .bind(next.title, next.dueDate, next.status, next.ownerUserId, next.establishmentId, next.notes, next.protocol, next.evidenceUrl, next.status, workspace.id, id),
