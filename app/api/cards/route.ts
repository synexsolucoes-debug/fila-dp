...
export async function GET(request: Request) {
  ...
  try {
    ...
    const result = await d1.prepare(`SELECT card.id, card.reference_number, card.title, card.description,
        ...
        (SELECT COUNT(*)::int FROM fdp_checklist_items task WHERE task.workspace_id = card.workspace_id AND task.card_id = card.id) AS tasks_total,
        (SELECT COUNT(*)::int FROM fdp_checklist_items task WHERE task.workspace_id = card.workspace_id AND task.card_id = card.id AND task.completed = 1) AS tasks_done
