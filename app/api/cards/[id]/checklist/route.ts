...
export async function POST(request: Request, context: RouteContext) {
  ...
  try {
    ...
    const card = await d1.prepare("SELECT id FROM cards WHERE id = ? AND board_id = ? AND archived = 0").bind(id, board.id).first();
    if (!card) throw new Error("Demanda não encontrada.");
    const position = await d1.prepare("SELECT COALESCE(MAX(position), 0) AS max_position FROM checklist_items WHERE card_id = ?").bind(id).first<{ max_position: number }>();
    await d1.prepare("INSERT INTO checklist_items (id, card_id, title, completed, position) VALUES (?, ?, ?, 0, ?)")
