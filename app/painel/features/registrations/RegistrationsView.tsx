...
import { EmployeeExamsPanel } from "./EmployeeExamsPanel";
...
const riskLevelLabels: Record<CatalogItem["riskLevel"], string> = {
...
const emptyEmployee: EmployeeDraft = {
  ...
  workScheduleId: "", establishmentId: "", managerEmployeeId: "", admissionDate: "", birthDate: "", terminationDate: "", employmentStatus: "active", employmentType: "clt",
...
function normalizeEmployee(raw: JsonRecord): Employee {
  ...
  return {
    ...
    establishmentId: nullable(key(raw, "establishmentId", "establishment_id")), establishmentName: text(key(raw, "establishmentName", "establishment_name")),
...
function normalizeCatalog(raw: JsonRecord): CatalogItem {
  const riskLevel = text(key(raw, "riskLevel", "risk_level"));
  return {
    ...
    cboCode: text(key(raw, "cboCode", "cbo_code")),
    riskLevel: (["low", "medium", "high"].includes(riskLevel) ? riskLevel : "none") as CatalogItem["riskLevel"],
    specialActivities: text(key(raw, "specialActivities", "special_activities")),
    weeklyHours: Number(key(raw, "weeklyHours", "weekly_hours")) || null,
...
function dateLabel(value: string) { if (!value) return "Não informado"; const date = new Date(value.includes("T") ? value : `${value}T12:00:00`); return Number.isNaN(date.getTime()) ? value : new Intl.DateTimeFormat("pt-BR", { day: "2-digit", month: "2-digit", year: "numeric" }).format(date); }
...
export function RegistrationsView({ role, initialEmployeeId = "", initialEmployeeTab = "", onOpenContractorPayment }: {
  ...
  const canManageExams = hasCapability(role, "exams.manage");
  const canManageTrainings = hasCapability(role, "trainings.manage");
  ...
  /** Dossiê do colaborador, passo 1 (§4.18): EPI, ASO e treinamento num PDF só. */
  async function downloadEmployeeDossier(employee: Employee) {
    setBusy(true);
    try {
      const response = await fetch(`/api/employees/${employee.id}/dossier`);
      if (!response.ok) {
        const payload = await response.json().catch(() => ({})) as { error?: string; message?: string };
        throw new Error(payload.message || payload.error || "Não foi possível gerar o dossiê.");
      }
      const blob = await response.blob();
      const url = URL.createObjectURL(blob);
      const link = document.createElement("a");
      link.href = url;
      link.download = `dossie-${employee.registrationNumber || employee.id}.pdf`;
      document.body.appendChild(link);
      link.click();
      link.remove();
      URL.revokeObjectURL(url);
      setError(""); setToast(`Dossiê de ${employee.socialName || employee.fullName} gerado.`);
    } catch (cause) { setError(cause instanceof Error ? cause.message : "Não foi possível gerar o dossiê."); }
  ...
  return (
    <section className={styles.root} aria-label="Cadastros operacionais">
      ...
      {employeeEditor && <EmployeeDrawer employee={employeeEditor === "new" ? null : employeeEditor} companies={companies} catalogs={catalogs} busy={busy} editing={employeeEditing} activeTab={employeeDetailTab}
        history={history} historyLoading={historyLoading} canManage={canManageRegistrations} canConsultTangerino={canConsultTangerino} canManageExams={canManageExams} canManageTrainings={canManageTrainings} onTab={(next) => { if (next === "history" && employeeEditor !== "new") void openHistory(employeeEditor); else setEmployeeDetailTab(next); }}
        ...
        onDownloadDossier={downloadEmployeeDossier}
...
function CatalogEditor({ resource, item, company, busy, onClose, onBusy, onError, onSaved, onInactivate }: { resource: CatalogResource; item: CatalogItem | null; company: Company; busy: boolean; onClose: () => void; onBusy: (value: boolean) => void; onError: (value: string) => void; onSaved: () => Promise<void>; onInactivate: (item: CatalogItem) => void }) {
  const [code, setCode] = useState(item?.code ?? ""); const [name, setName] = useState(item?.name ?? ""); const [cboCode, setCboCode] = useState(item?.cboCode ?? ""); const [riskLevel, setRiskLevel] = useState<CatalogItem["riskLevel"]>(item?.riskLevel ?? "none"); const [specialActivities, setSpecialActivities] = useState(item?.specialActivities ?? ""); const [weeklyHours, setWeeklyHours] = useState(String(item?.weeklyHours ?? 44)); const [description, setDescription] = useState(item?.description ?? "");
  async function submit(event: FormEvent) { event.preventDefault(); onBusy(true); try { await requestJson(item ? `/api/registrations/catalogs/${resource}/${item.id}` : `/api/registrations/catalogs/${resource}`, { method: item ? "PATCH" : "POST", body: JSON.stringify({ companyId: company.id, code, name, cboCode, riskLevel, specialActivities, weeklyHours: Number(weeklyHours), description }) }); await onSaved(); } catch (cause) { onError(cause instanceof Error ? cause.message : "Não foi possível salvar o cadastro."); } finally { onBusy(false); } }
  return <div className={styles.overlay} onMouseDown={(event) => { if (event.target === event.currentTarget) onClose(); }}><form className={styles.compactEditor} onSubmit={submit}><header><div><span>{displayCompany(company)}</span><h2>{item ? "Editar" : "Novo"} {catalogMeta[resource].singular.toLowerCase()}</h2></div><button type="button" onClick={onClose}><X /></button></header><div className={styles.editorBody}><label><span>Código *</span><input required value={code} onChange={(event) => setCode(event.target.value)} /></label><label><span>Nome *</span><input required value={name} onChange={(event) => setName(event.target.value)} /></label>{resource === "positions" && <label><span>Código CBO</span><input value={cboCode} onChange={(event) => setCboCode(event.target.value)} /></label>}{resource === "positions" && <label><span>Grau de risco</span><select value={riskLevel} onChange={(event) => setRiskLevel(event.target.value as CatalogItem["riskLevel"])}><option value="none">Sem risco</option><option value="low">Baixo</option><option value="medium">Médio</option><option value="high">Alto</option></select></label>}{resource === "positions" && <label><span>Atividades especiais</span><input value={specialActivities} onChange={(event) => setSpecialActivities(event.target.value)} placeholder="Ex.: trabalho em altura, espaço confinado" /></label>}{resource === "work-schedules" && <><label><span>Carga semanal</span><input type="number" min="1" max="60" value={weeklyHours} onChange={(event) => setWeeklyHours(event.target.value)} /></label><label><span>Descrição da jornada</span><textarea value={description} onChange={(event) => setDescription(event.target.value)} /></label></>}</div><footer>{item?.status === "active" && <button type="button" className={styles.textDanger} onClick={() => onInactivate(item)}>Inativar</button>}<span /><button type="button" className={styles.secondaryButton} onClick={onClose}>Cancelar</button><button className={styles.primaryButton} disabled={busy}>{busy ? <LoaderCircle className={styles.spin} /> : <Check />} Salvar</button></footer></form></div>;
}

/**
 * Portal do Gestor, passo 1 (§4.20): `manager_employee_id` existe desde a
 * fundação dos cadastros, mas nunca teve onde ser preenchido — só a rota
 * aceitava o campo. Busca em vez de lista, pelo mesmo motivo do vínculo
 * conta↔colaborador (WorkspaceApp.tsx#MemberEmployeeLink): o cadastro de uma
 * empresa pode ter milhares de colaboradores.
 */
function EmployeeManagerPicker({ companyId, employeeId, value, readOnly, onChange }: { companyId: string; employeeId: string; value: string; readOnly: boolean; onChange: (id: string) => void }) {
  const [query, setQuery] = useState("");
  const [results, setResults] = useState<Array<{ id: string; fullName: string; registrationNumber: string }>>([]);
  const [currentName, setCurrentName] = useState("");
  const [searching, setSearching] = useState(false);
  useEffect(() => {
    let active = true;
    const frame = window.requestAnimationFrame(() => {
      if (!value) { setCurrentName(""); return; }
      void requestJson<{ employee?: JsonRecord }>(`/api/employees/${value}`)
        .then((payload) => { if (active && payload.employee) setCurrentName(text(key(payload.employee, "fullName", "full_name"))); })
        .catch(() => { if (active) setCurrentName(""); });
    });
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(() => {
      ...
      if (trimmed.length < 2 || !companyId) { setResults([]); return; }
      setSearching(true);
      void fetch(`/api/employees?search=${encodeURIComponent(trimmed)}&companyId=${encodeURIComponent(companyId)}&status=active&limit=8`, { signal: controller.signal })
        .then((response) => response.ok ? response.json() as Promise<{ employees?: JsonRecord[] }> : { employees: [] })
        .then((payload) => setResults((payload.employees ?? [])
          .filter((row) => text(row.id) !== employeeId)
          .map((row) => ({ id: text(row.id), fullName: text(key(row, "fullName", "full_name")), registrationNumber: text(key(row, "registrationNumber", "registration_number")) }))))
        .catch(() => setResults([]))
        .finally(() => setSearching(false));
    }, 300);
    return () => { window.clearTimeout(timer); controller.abort(); };
  }, [query, companyId, employeeId]);
  if (readOnly) return <span>{value ? (currentName || "Vinculado") : "Não informado"}</span>;
  return <div className={styles.managerPicker}>
    {value && <div className={styles.managerPickerCurrent}><span>{currentName || "Selecionado"}</span><button type="button" onClick={() => onChange("")}>Remover</button></div>}
    <input type="search" placeholder="Buscar por nome ou matrícula" value={query} onChange={(event) => setQuery(event.target.value)} />
    {searching && <small>Buscando…</small>}
    {!searching && query.trim().length >= 2 && results.length === 0 && <small>Nenhum colaborador encontrado.</small>}
    <ul>{results.map((result) => <li key={result.id}><button type="button" onClick={() => { onChange(result.id); setQuery(""); setResults([]); }}>{result.fullName} · {result.registrationNumber || "sem matrícula"}</button></li>)}</ul>
  </div>;
}
...
function EmployeeDrawer({
  ...
  return <div className={styles.drawerOverlay} onMouseDown={(event) => { if (event.target === event.currentTarget) onClose(); }}><aside className={styles.employeeDrawer} role="dialog" aria-modal="true" aria-labelledby="employee-title"><header className={styles.drawerHeader}><div className={styles.employeeHero}><span>{employee ? initials(employee.socialName || employee.fullName) : <UserRound />}</span><div><small>{employee ? `MATRÍCULA ${employee.registrationNumber}` : "NOVO CADASTRO MANUAL"}</small><h2 id="employee-title">{employee ? employee.socialName || employee.fullName : "Novo colaborador"}</h2><p>{employee ? `${employee.positionName || "Sem cargo"} · ${employee.companyName}` : "Registre uma admissão já concluída na origem."}</p></div></div><div>{employee && <button className={styles.secondaryButton} onClick={() => onDownloadDossier(employee)} disabled={busy}><FileDown /> Baixar dossiê</button>}{employee && canManage && !editing && <button className={styles.secondaryButton} onClick={onEdit}><Edit3 /> Editar</button>}<button className={styles.closeButton} onClick={onClose} aria-label="Fechar"><X /></button></div></header><nav className={styles.drawerTabs}>{(["personal", "employment", "contact", "epi", "exams", "trainings", "tangerino", "history"] as EmployeeDetailTab[]).map((item) => <button key={item} className={activeTab === item ? styles.drawerTabActive : ""} onClick={() => onTab(item)} disabled={!employee && item !== "personal" && item !== "employment" && item !== "contact"}>{item === "personal" ? "Dados pessoais" : item === "employment" ? "Vínculo e lotação" : item === "contact" ? "Contato" : item === "epi" ? "EPIs" : item === "exams" ? "Exames (ASO)" : item === "trainings" ? "Treinamentos (NR)" : item === "tangerino" ? "Tangerino" : "Histórico"}</button>)}</nav>
      <form onSubmit={submit} className={styles.drawerForm}><div className={styles.drawerBody}>
        ...
        {activeTab === "exams" && employee && <EmployeeExamsPanel employeeId={employee.id} companyId={employee.companyId} canManage={canManageExams} />}
        {activeTab === "trainings" && employee && <EmployeeTrainingsPanel employeeId={employee.id} companyId={employee.companyId} canManage={canManageTrainings} />}
