...
import {
  AlertOctagon, AlertTriangle, ArrowRight, BookOpenCheck, Building2, CalendarClock, Check, CheckCircle2,
  ChevronRight, CircleDashed, CircleDot, ClipboardCheck, FilePlus2, Gauge, Inbox, ListChecks, LoaderCircle,
  LockKeyhole, Plus, RefreshCw, Search, ShieldAlert, ShieldCheck, TimerReset, UserCheck, UsersRound, XCircle,
} from "lucide-react";
...
export function OperationsView({ role }: { role: WorkspaceRole }) {
  ...
  async function handleDialogSubmit(state: NonNullable<EditorState>, form: FormData) {
    ...
    try {
      if (state.kind === "competence") {
        ...
        await mutate("/api/operations/obligations", { method: "POST", body: JSON.stringify({ companyId, competenceId: cycle?.id, obligationType: field(form, "obligationType"), title: field(form, "title"), dueDate: field(form, "dueDate"), ownerUserId: field(form, "ownerUserId") || null, cardId: field(form, "cardId") || null, notes: field(form, "notes") }) }, "Obrigação incluída no ciclo.");
  ...
  return (
    <section className={styles.workspace} aria-label="Operação do Departamento Pessoal">
      ...
      {error && <div className={styles.errorBanner} role="alert"><AlertOctagon aria-hidden="true" /><span><strong>Algo exige atenção</strong>{error}</span><button type="button" onClick={() => setError("")} aria-label="Fechar aviso"><XCircle aria-hidden="true" /></button></div>}
...
function PanelHeader({ eyebrow, title, description, action }: { eyebrow: string; title: string; description: string; action?: React.ReactNode }) { return <header className={styles.panelHeader}><div><span className={styles.eyebrow}>{eyebrow}</span><h2>{title}</h2><p>{description}</p></div>{action}</header>; }
function EmptyState({ icon: Icon, title, text, action }: { icon: typeof Gauge; title: string; text: string; action?: React.ReactNode }) { return <div className={styles.emptyState}><span><Icon aria-hidden="true" /></span><strong>{title}</strong><p>{text}</p>{action}</div>; }
function StatusPill({ status }: { status: string }) { const tone = ["closed", "completed", "approved", "resolved", "published"].includes(status) ? "safe" : ["blocked", "rejected", "critical"].includes(status) ? "danger" : ["pending_approval", "in_progress", "warning"].includes(status) ? "warning" : "neutral"; return <span className={styles.statusPill} data-tone={tone}><CircleDot aria-hidden="true" />{statusLabels[status] ?? status}</span>; }
...
function ApprovalsPanel({ approvals, canDecide, onDecide }: { approvals: OverviewPayload["approvals"]; canDecide: boolean; onDecide: (item: OverviewPayload["approvals"][number]) => void }) {
  return <><PanelHeader eyebrow="GOVERNANÇA" title="Fila de aprovações" description="Decisões atribuídas com segregação para salário e desligamento." />{approvals.length ? <div className={styles.approvalQueue}>{approvals.map((item) => <article key={item.id} className={item.status === "pending" ? styles.approvalPending : ""}><span className={styles.approvalSequence}>{String(item.sequence).padStart(2, "0")}</span><div><small>{item.employeeName || "COLABORADOR"}</small><strong>{item.movementTitle}</strong><StatusPill status={item.status} /></div>{item.status === "pending" && canDecide && item.canDecide ? <button className={styles.primaryButton} onClick={() => onDecide(item)}>Decidir <ArrowRight aria-hidden="true" /></button> : <span className={styles.decisionDate}>{item.decidedAt ? isoDate(item.decidedAt) : item.canDecide ? "Aguardando decisão" : "Aguardando responsável"}</span>}</article>)}</div> : <EmptyState icon={UserCheck} title="Fila de aprovações limpa" text="Novas solicitações atribuídas aparecerão aqui para decisão." />}</>;
}
...
function ClosingPanel({ items, canManage, onStatus }: { items: ClosingItem[]; canManage: boolean; onStatus: (id: string, status: WorkStatus) => void }) {
  ...
  return <><PanelHeader eyebrow="CHECKLIST CONTROLADO" title="Fechamento em duas frentes" description="Cada gate mantém estado, responsável e evidência temporal." /><div className={styles.closingColumns}>{phases.map((phase) => { const phaseItems = items.filter((item) => item.phase === phase.id); const complete = phaseItems.filter((item) => item.status === "completed").length; return <section key={phase.id} className={styles.closingColumn}><header><div><span className={styles.eyebrow}>{phase.eyebrow}</span><h3>{phase.title}</h3></div><strong>{complete}/{phaseItems.length}</strong></header>{phaseItems.length ? <ol>{phaseItems.map((item) => <li key={item.id}><span className={item.status === "completed" ? styles.checkDone : item.status === "blocked" ? styles.checkBlocked : ""}>{item.status === "completed" ? <Check aria-hidden="true" /> : item.status === "blocked" ? <LockKeyhole aria-hidden="true" /> : <CircleDot aria-hidden="true" />}</span><div><strong>{item.title}</strong><small>{item.category} {item.dueDate ? `· ${isoDate(item.dueDate)}` : ""}</small></div>{canManage ? <select aria-label={`Status de ${item.title}`} value={item.status} onChange={(event) => onStatus(item.id, event.target.value as WorkStatus)} disabled={false}><option value="pending">Pendente</option><option value="in_progress">Em andamento</option><option value="blocked">Bloqueado</option><option value="completed">Concluído</option></select> : <StatusPill status={item.status} />}</li>)}</ol> : <EmptyState icon={ListChecks} title="Checklist indisponível" text="Os itens padrão são criados quando a competência é aberta." />}</section>; })}</div></>;
}
...
function ObligationsPanel({ items, canManage, onCreate, onStatus }: { items: Obligation[]; canManage: boolean; onCreate: () => void; onStatus: (id: string, status: Obligation["status"]) => void }) {
  return <><PanelHeader eyebrow="CALENDÁRIO LEGAL" title="Obrigações da competência" description="Prazos, responsáveis e vínculo opcional com demandas." action={canManage && <button className={styles.primaryButton} onClick={onCreate}><Plus aria-hidden="true" /> Nova obrigação</button>} />{items.length ? <div className={styles.obligationList}>{items.map((item) => { const days = daysUntil(item.dueDate); const late = item.status !== "completed" && (days ?? 0) < 0; return <article key={item.id} className={late ? styles.obligationLate : ""}><time><strong>{item.dueDate?.slice(8, 10)}</strong><span>{new Date(`${item.dueDate}T12:00:00`).toLocaleDateString("pt-BR", { month: "short" })}</span></time><div><small>{obligationLabels[item.obligationType] ?? item.obligationType}</small><strong>{item.title}</strong><span>{item.cardId ? "Vinculada a uma demanda" : "Sem demanda vinculada"}</span></div><em>{late ? `${Math.abs(days ?? 0)}d atrasada` : days === 0 ? "Vence hoje" : days != null ? `em ${days}d` : "Sem prazo"}</em>{canManage ? <select value={item.status} onChange={(event) => onStatus(item.id, event.target.value as Obligation["status"])} aria-label={`Status de ${item.title}`}><option value="open">Aberta</option><option value="in_progress">Em andamento</option><option value="blocked">Bloqueada</option><option value="completed">Concluída</option></select> : <StatusPill status={item.status} />}</article>; })}</div> : <EmptyState icon={CalendarClock} title="Nenhuma obrigação no ciclo" text="Inclua prazos legais para que o cockpit antecipe riscos." action={canManage && <button className={styles.secondaryButton} onClick={onCreate}>Criar obrigação</button>} />}</>;
}
...
function LoadingState() { return <section className={styles.workspace}><div className={styles.loadingState}><LoaderCircle className={styles.spin} aria-hidden="true" /><strong>Montando o cockpit de fechamento</strong><span>Carregando competências, gates e exceções…</span></div></section>; }
