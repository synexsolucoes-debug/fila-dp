...
import { Download, Eye, Focus, Maximize2, Plus, Redo2, Save, Trash2, Undo2, ZoomIn, ZoomOut } from "lucide-react";
import type { ProcessArea, ProcessCondition, ProcessDefinition, ProcessMember, ProcessStepConfig, ProcessTaskTemplate, ProcessVersionDetail } from "./processes.types";
...
function TaskTemplateRows({rows,onChange,readOnly,areas,members}:{rows:ProcessTaskTemplate[];onChange:(next:ProcessTaskTemplate[])=>void;readOnly:boolean;areas:ProcessArea[];members:ProcessMember[]}){
  const update=(id:string,patch:Partial<ProcessTaskTemplate>)=>onChange(rows.map((row)=>row.id===id?{...row,...patch}:row));
  const add=()=>onChange([...rows,{id:crypto.randomUUID(),title:"",instructions:"",responsibilityMode:"INHERIT",responsibleUserId:"",responsibleAreaId:"",evidenceRequired:false}]);
  return <div className={styles.taskTemplates}>
    {rows.length===0&&<p className={styles.conditionEmpty}>Sem tarefas detalhadas. Se a etapa tiver checklist, cada item será materializado como tarefa.</p>}
    {rows.map((row,index)=><article className={styles.taskTemplate} key={row.id}>
      <header><strong>Tarefa {index+1}</strong><button type="button" aria-label={`Remover tarefa ${index+1}`} disabled={readOnly} onClick={()=>onChange(rows.filter((item)=>item.id!==row.id))}><Trash2/></button></header>
      <label>Título<input required value={row.title} placeholder="Ex.: Conferir documentos admissionais" onChange={(e)=>update(row.id,{title:e.target.value})}/></label>
      <label>Instruções<textarea rows={3} value={row.instructions} placeholder="Explique como executar e qual é o resultado esperado" onChange={(e)=>update(row.id,{instructions:e.target.value})}/></label>
      <label>Responsabilidade<select value={row.responsibilityMode} onChange={(e)=>update(row.id,{responsibilityMode:e.target.value})}><option value="INHERIT">Herdar da etapa</option><option value="USER">Usuário específico</option><option value="DEPARTMENT">Área específica</option><option value="REQUESTER">Solicitante</option><option value="EMPLOYEE_MANAGER">Gestor do colaborador</option><option value="PROCESS_OWNER">Dono do processo</option></select></label>
      {row.responsibilityMode==="USER"&&<label>Usuário<select value={row.responsibleUserId} onChange={(e)=>update(row.id,{responsibleUserId:e.target.value})}><option value="">Selecione</option>{members.map((member)=><option key={member.id} value={member.id}>{member.name}</option>)}</select></label>}
      {row.responsibilityMode==="DEPARTMENT"&&<label>Área<select value={row.responsibleAreaId} onChange={(e)=>update(row.id,{responsibleAreaId:e.target.value})}><option value="">Selecione</option>{areas.map((area)=><option key={area.id} value={area.id}>{area.name}</option>)}</select></label>}
      <label className={styles.checkLabel}><input type="checkbox" checked={row.evidenceRequired} onChange={(e)=>update(row.id,{evidenceRequired:e.target.checked})}/> Exigir evidência para concluir</label>
    </article>)}
    <button type="button" className={styles.taskAdd} disabled={readOnly||rows.length>=50} onClick={add}><Plus/>Adicionar tarefa</button>
  </div>;
}
  ...
  return <div className={styles.modelerShell} ref={shellRef}>
    ...
    <div className={styles.modelerGrid}><section className={styles.canvasPanel}><div ref={canvasRef} className={styles.bpmnCanvas} tabIndex={0}/></section><aside className={styles.propertiesPanel}>
      {!selected?<div className={styles.propertiesEmpty}><WorkflowHint/></div>:<><header className={styles.propertiesHeader}><span>{selected.type.replace("bpmn:","")}</span><strong>{selected.name||selected.id}</strong><small>{selected.id}</small></header><div className={styles.propertySections}>
        ...
        <fieldset disabled={readOnly}><legend>Tarefas da etapa</legend><p className={styles.fieldsetHint}>Estas tarefas viram instâncias independentes em cada demanda, com responsável, status, instruções e evidência próprios.</p><TaskTemplateRows readOnly={readOnly} rows={config?.settings.tasks??[]} areas={areas} members={members} onChange={(next)=>patchSettings({tasks:next})}/></fieldset>
