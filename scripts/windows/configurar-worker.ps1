...
Write-Host "  Se la existir FDP_TANGERINO_VAULT_KEYS preenchido, deixe este em"
Write-Host "  branco e cole o mapa quando for perguntado em seguida."
...
$vaultKeys = ""
do {
  $vaultKey = (Read-Host "  FDP_TANGERINO_VAULT_KEY (Enter para pular)").Trim()
  if ([string]::IsNullOrWhiteSpace($vaultKey)) {
    $vaultKeys = (Read-Host "  FDP_TANGERINO_VAULT_KEYS (o mapa JSON)").Trim()
    $valido = $vaultKeys.StartsWith("{") -and $vaultKeys.EndsWith("}")
    if (-not $valido) { Aviso "O mapa precisa ser um JSON, como {`"1`":`"...`"}." }
  } else {
    $valido = Chave32Bytes $vaultKey
    if (-not $valido) {
