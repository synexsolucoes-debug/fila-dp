...
export async function getWorkspaceSnapshot(user: ChatGPTUser): Promise<WorkspaceSnapshot> {
  ...
  const [boardsResult, listsResult, allBoardListsResult, cardsResult, checklistResult, inboxResult, rulesResult, commentsResult, activitiesResult, membersResult, workspacesResult, labelsResult, cardLabelsResult, assigneesResult, customFieldsResult, customValuesResult, attachmentsResult, templatesResult, settingsRow, holidaysResult, policiesResult, integrationsResult, plannerResult, calendarsResult, companiesResult, hrMetricsResult, employmentsResult, benefitPoliciesResult, benefitMovementsResult, pjClosingsResult, pausesResult] = await Promise.all([
    ...
    d1.prepare(`SELECT e.id, e.person_id, e.company_id, e.employee_code, e.regime, e.job_title, e.department, e.cost_center, e.manager_name,
      e.start_date, e.end_date, e.monthly_value, e.status, e.source, e.external_id, p.full_name, p.preferred_name, p.email, p.phone
      FROM fdp_employments e JOIN fdp_people p ON p.id = e.person_id
      WHERE e.workspace_id = ? ORDER BY p.full_name, e.start_date DESC`).bind(workspace.id).all(),
    d1.prepare("SELECT id, company_id, name, benefit_type, eligible_regime, monthly_value, employee_discount, channel, effective_from, effective_to, active FROM fdp_benefit_policies WHERE workspace_id = ? ORDER BY active DESC, name").bind(workspace.id).all(),
    d1.prepare("SELECT id, policy_id, employment_id, company_id, period, amount, employee_discount, status, notes FROM fdp_benefit_movements WHERE workspace_id = ? ORDER BY period DESC, created_at DESC LIMIT 1000").bind(workspace.id).all(),
    d1.prepare("SELECT id, employment_id, company_id, period, contract_amount, variable_amount, reimbursement_amount, deductions_amount, invoice_limit, invoice_amount, caju_excess, net_amount, status, notes FROM fdp_pj_closings WHERE workspace_id = ? ORDER BY period DESC, created_at DESC LIMIT 1000").bind(workspace.id).all(),
  ...
  const canViewPeople = workspace.role !== "guest";
  const visibleEmploymentRows = canViewPeople ? (employmentsResult.results as Array<Record<string, unknown>>).filter((row) => isVisibleCompany(row.company_id)) : [];
  const visibleBenefitPolicyRows = canViewPeople ? (benefitPoliciesResult.results as Array<Record<string, unknown>>).filter((row) => isVisibleCompany(row.company_id)) : [];
  const visibleBenefitMovementRows = canViewPeople ? (benefitMovementsResult.results as Array<Record<string, unknown>>).filter((row) => isVisibleCompany(row.company_id)) : [];
  const visiblePjClosingRows = canViewPeople ? (pjClosingsResult.results as Array<Record<string, unknown>>).filter((row) => isVisibleCompany(row.company_id)) : [];
  const canViewFinancial = workspace.role === "admin" || workspace.role === "member";
  const canViewPersonalContact = workspace.role !== "guest";
  ...
  return {
    ...
    employments: visibleEmploymentRows.map((row) => ({ id: String(row.id), personId: String(row.person_id), companyId: String(row.company_id), fullName: String(row.full_name), preferredName: String(row.preferred_name ?? ""), email: canViewPersonalContact ? String(row.email ?? "") : "", phone: canViewPersonalContact ? String(row.phone ?? "") : "", employeeCode: String(row.employee_code ?? ""), regime: String(row.regime ?? "clt") as "clt" | "pj" | "intern" | "temporary", jobTitle: String(row.job_title ?? ""), department: String(row.department ?? ""), costCenter: String(row.cost_center ?? ""), managerName: String(row.manager_name ?? ""), startDate: row.start_date ? String(row.start_date) : null, endDate: row.end_date ? String(row.end_date) : null, monthlyValue: canViewFinancial ? Number(row.monthly_value ?? 0) : 0, status: String(row.status ?? "active") as "active" | "inactive" | "on_leave", source: String(row.source ?? "manual"), externalId: String(row.external_id ?? "") })),
    benefitPolicies: visibleBenefitPolicyRows.map((row) => ({ id: String(row.id), companyId: String(row.company_id), name: String(row.name), benefitType: String(row.benefit_type), eligibleRegime: String(row.eligible_regime ?? "all") as "all" | "clt" | "pj" | "intern" | "temporary", monthlyValue: canViewFinancial ? Number(row.monthly_value ?? 0) : 0, employeeDiscount: canViewFinancial ? Number(row.employee_discount ?? 0) : 0, channel: String(row.channel ?? "payroll"), effectiveFrom: row.effective_from ? String(row.effective_from) : null, effectiveTo: row.effective_to ? String(row.effective_to) : null, active: Boolean(row.active) })),
    benefitMovements: visibleBenefitMovementRows.map((row) => ({ id: String(row.id), policyId: String(row.policy_id), employmentId: String(row.employment_id), companyId: String(row.company_id), period: String(row.period), amount: canViewFinancial ? Number(row.amount ?? 0) : 0, employeeDiscount: canViewFinancial ? Number(row.employee_discount ?? 0) : 0, status: String(row.status ?? "calculated") as "calculated" | "approved" | "exported" | "cancelled", notes: canViewFinancial ? String(row.notes ?? "") : "" })),
    pjClosings: visiblePjClosingRows.map((row) => ({ id: String(row.id), employmentId: String(row.employment_id), companyId: String(row.company_id), period: String(row.period), contractAmount: canViewFinancial ? Number(row.contract_amount ?? 0) : 0, variableAmount: canViewFinancial ? Number(row.variable_amount ?? 0) : 0, reimbursementAmount: canViewFinancial ? Number(row.reimbursement_amount ?? 0) : 0, deductionsAmount: canViewFinancial ? Number(row.deductions_amount ?? 0) : 0, invoiceLimit: canViewFinancial ? Number(row.invoice_limit ?? 0) : 0, invoiceAmount: canViewFinancial ? Number(row.invoice_amount ?? 0) : 0, cajuExcess: canViewFinancial ? Number(row.caju_excess ?? 0) : 0, netAmount: canViewFinancial ? Number(row.net_amount ?? 0) : 0, status: String(row.status ?? "draft") as "draft" | "review" | "approved" | "paid" | "blocked", notes: canViewFinancial ? String(row.notes ?? "") : "" })),
